 ...
 *
 *   bun run scripts/sync-dashboard-api-key-samples.ts
...
/** Map route folder → dashboard operation key */
const OPS = [
...
/** Sample id in x-codeSamples → dashboard language key */
const LANGS = [
	...
	if (prev !== next) {
		console.error(
			"Dashboard api-keys-code-examples.ts is out of date with backend x-codeSamples.\n" +
				"Run: cd apps/frontend/docs && bun run sync:sdk-samples",
		);
		process.exit(1);
	}
