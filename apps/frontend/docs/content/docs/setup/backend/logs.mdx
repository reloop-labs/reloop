...
<Callout type="info" title="Complete Base Setup First" variant="note">
  Ensure you've finished the [Setup](/docs/setup) and have ClickHouse running (`bun docker:up`).
</Callout>
...
`CLICKHOUSE_URL` is a full connection string. Host, credentials, and database are parsed from it — there are no separate `CLICKHOUSE_USER` / `CLICKHOUSE_PASSWORD` / `CLICKHOUSE_DATABASE` env vars.
