...
The relational subscriber endpoints configuration, event subscriptions, payload archives, and dispatch delivery logs are defined inside [packages/db/src/schema/webhook.ts](file:///Users/pranavpatel/Downloads/reloop/packages/db/src/schema/webhook.ts).

### Enums

| Enum | Values | Description |
| :--- | :--- | :--- |
| `webhook_status` | `active` · `paused` · `disabled` · `failed` | Webhook subscription delivery status |
| `webhook_delivery_status` | `pending` · `success` · `failed` · `retrying` | Specific event dispatch execution status |
...
| `name` | `varchar(255)` | — | Friendly name label for the endpoint |
| `url` | `text` | — | Recipient HTTP target URL destination |
| `secret` | `text` | — | Cryptographic HMAC-SHA256 signature signing token |
...
| `webhookId` | `text` | — | Foreign Key &rarr; [webhook.id](file:///Users/pranavpatel/Downloads/reloop/packages/db/src/schema/webhook.ts) |
| `webhookEventId` | `text` | `null` | Foreign Key &rarr; [webhook_event.id](file:///Users/pranavpatel/Downloads/reloop/packages/db/src/schema/webhook.ts) |
| `eventType` | `text` | — | Semantic event type (e.g. `domain.verified`) |
| `status` | `webhook_delivery_status`| `pending` | Dispatch attempt resolution state |
| `requestUrl` | `text` | — | Dispatched network URL |
| `requestHeaders` | `jsonb` | `null` | Sent headers including security signature |
| `requestBody` | `jsonb` | — | Sent JSON body payload |
...
* **`webhook_event_subscription`**: Link table mapping [webhooks](file:///Users/pranavpatel/Downloads/reloop/packages/db/src/schema/webhook.ts) to specific event filters.
