...
| `APP_NAME` | No | `Reloop` | Product name used in system email senders, subjects and bodies |


## Branding system email

`APP_NAME` sets the product name used across every system email — the sender
name, the subject line and the message body — so a self-hosted install can send
mail under its own name instead of Reloop's.
...
| :--- | :--- |
| Login code | `Contoso Mail <auth@…>` |
| New sign-in | `Contoso Mail Security <security@…>` |
| Billing | `Contoso Mail Billing <billing@…>` |
| Quota warning | `Contoso Mail Support <support@…>` |
| Invitation | `Dana via Contoso Mail <invites@…>` |

Leave it unset and everything stays `Reloop`, so existing deployments are
unaffected. The installer defaults it to `Reloop` and preserves your value
across re-runs.
...
> [!NOTE]
> `APP_NAME` does not change the shared footer, which keeps Reloop's help
> link, copyright line and postal address. That attribution is retained
> deliberately, so every install sends mail with it regardless of
> `APP_NAME`.
