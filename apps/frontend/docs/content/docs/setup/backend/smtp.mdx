...
<Tip>
Run [`bun setup`](/docs/setup) for core Docker first. SMTP is on the **edge** profile — start it with `bun docker:up:edge` or `bun setup --with-edge` (needs free ports `465` / `587`).
</Tip>
...
View captured mail at [http://localhost:8025](http://localhost:8025) (Mailpit, core stack).
...
| `docker compose -f local/docker-compose.yml --profile edge up -d smtp` | Start SMTP only |
