...
<Tip>
Run [`bun setup`](/docs/setup) once from the monorepo root. For real SMTP delivery locally, also start the edge stack: `bun docker:up:edge` (see [SMTP](/docs/setup/backend/smtp)). Mailpit still captures mail without edge for many flows.
</Tip>
