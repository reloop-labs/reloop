...
- **NATS** (`4222`): Core messaging bus for pub-sub between services.
- **Mailpit** (`8025`): Local SMTP sandbox and web UI for captured mail.
- **MinIO** (`9010` / `9011`): S3-compatible object storage for uploads and attachments.
- **RSpamD** (`11333`): Spam filtering for inbound mail.
- **SMTP** (`587` / `2025` / `8020`): Outbound KumoMTA mail engine (Docker).
- **Inbound** (`25` / `8030`): Inbound KumoMTA MX receiver (Docker).
...
| [https://local.reloop.sh/preferences](https://local.reloop.sh/preferences) | **Links App** | `3005` | Contact preference center |
| [https://local.reloop.sh/redirect](https://local.reloop.sh/redirect) | **Links App** | `3005` | Tracked link redirects |
...
| **Links App** | [http://localhost:3005](http://localhost:3005) | `3005` | Preference center and tracked link redirects |
...
| **Mail API** | `/api/mail` | `8015` | Campaign engine, SMTP queues, and sending pipeline |
...
| **Inbox API** | `/api/inbox` | `8021` | Agent inbox mailboxes, threads, and messages |
| **Email API** | `/api/email` | `8022` | Platform transactional email handler |
| **Credits API** | `/api/credits` | `8023` | Stripe subscriptions and credit balance management |
| **SMTP Engine** | Docker HTTP | `8020` | Outbound KumoMTA mail engine (no Caddy API route) |
| **Inbound MTA** | Docker HTTP | `8030` | Inbound KumoMTA MX receiver health endpoint |
...
| **Mailpit** | `8025` | Local SMTP capture and web UI |
...
<Cards>
  ...
  <Card icon={<Icon name="server" />} title="Backend Setup" href="/setup/backend/auth">
    Configure auth keys, credits, SMTP, webhooks, and mail engines.
  </Card>
  <Card icon={<Icon name="home" />} title="Frontend Setup" href="/setup/frontend/web">
    Configure web, dashboard, docs, and links apps.
  </Card>
</Cards>
