...
```bash
curl "https://api.reloop.sh/api/contacts/v1/list?page=2&limit=50" \
...
Here's a complete example for fetching all contacts across multiple pages:

```javascript
async function fetchAllContacts(apiKey) {
  const contacts = [];
  let page = 1;
  const limit = 100; // Use max page size for fewer requests

  while (true) {
    const res = await fetch(
      `https://api.reloop.sh/api/contacts/v1/list?page=${page}&limit=${limit}`,
