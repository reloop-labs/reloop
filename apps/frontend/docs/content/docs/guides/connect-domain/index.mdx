...
Reloop can send and receive email for **any domain you control** (receiving and tracking are on by default for new domains — turn receiving off if you keep another inbox on that hostname). You do not need to buy the domain from Reloop — register it anywhere, point DNS at the records Reloop shows you, and verify.
...
<Steps>
  ...
  <Step title="Verify">
    Click **Verify & finish** (setup) or **Verify Domain** (domain detail) to start checks. When status is **active**, you can send from that domain.
  </Step>
</Steps>
