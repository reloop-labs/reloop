...
<Steps>
  ...
  <Step title="Add each record">
    ...
    4. For MX, set **priority** (often `10`) — add **Receiving MX** (`inbound.reloop.sh`) only when Email Receiving is on
    5. Save
  ...
  <Step title="Verify in Reloop">
    Return to Reloop → **Verify & finish** (setup) or **Verify Domain** (detail). Allow time for [propagation](/docs/guides/connect-domain/verification/propagation) if checks fail immediately.
  </Step>
</Steps>
...
- [ ] DMARC published when Email Sending is on
- [ ] Email Receiving toggled to match intent ([sending vs receiving](/docs/guides/connect-domain/sending-vs-receiving)) — default is on for new domains
