...
<Steps>
  ...
  <Step title="Check receiving before Auto-populate">
    New domains start with **Email Receiving** on, so Auto-populate usually includes inbound MX (`inbound.reloop.sh`). If this hostname must keep Google Workspace / Microsoft 365, turn **Email Receiving** off first — or use a [separate subdomain](/docs/guides/connect-domain/what-is-a-domain). See [Sending vs receiving](/docs/guides/connect-domain/sending-vs-receiving).
  </Step>
  <Step title="Click Auto populate">
    Reloop opens NameSilo for consent. Sign in if asked, review the records, and approve.
  </Step>
  <Step title="Verify">
    Return to Reloop and click **Verify & finish** or **Verify Domain** to start checks. Status should move to **active** within a few minutes (see [propagation](/docs/guides/connect-domain/verification/propagation)).
  </Step>
</Steps>
...
<Info>
Omit your domain from the Name/Host field when the provider appends it automatically. Paste `reloop._domainkey` or `link` (or `link.<sub>` for subdomains), not the full FQDN ending in `.example.com`. Always copy the host from Reloop.
</Info>
...
<Steps>
  ...
  <Step title="Set the name / host">
    Copy the host from Reloop (`@`, `reloop._domainkey`, `_dmarc`, `link` or `link.<sub>`, …).
  </Step>
...
| DMARC | TXT | `_dmarc` | `v=DMARC1; p=reject;` (or your existing policy) — required when Email Sending is on |
| Sending MX | MX | `@` or subdomain host | `reloop.sh` (required when Email Sending is on) |
| Tracking | CNAME | `link` (or `link.<sub>` for subdomains) | `link.reloop.sh` (on by default for new domains) |
| Receiving MX | MX | `@` or subdomain host | `inbound.reloop.sh` (on by default for new domains — turn Email Receiving off if you keep another inbox here) |
...
2. Open the domain and click **Verify Domain** (setup page: **Verify & finish**; menu: **Re-verify DNS**).
...
<AccordionGroup>
  <Accordion title="Verification failed after I added records">
    Re-open the Reloop domain page and compare every Type / Host / Value to NameSilo. Fix typos, truncated DKIM keys, and duplicate SPF records. Then click **Verify Domain** again.
  </Accordion>
