---
title: Domain statuses
sidebarTitle: Domain statuses
description: What pending, verifying, active, failed, and suspended mean for a Reloop domain.
---

| Status | Meaning | What you should do |
| --- | --- | --- |
| **pending** | Domain created; DNS not confirmed yet | Publish records (auto or manual), then verify |
| **verifying** | Reloop is checking public DNS | Wait; re-check after propagation |
| **active** | Required records matched | Send mail; optionally [test](/docs/connect-domain/after/test-domain) |
| **failed** | Checks did not match expected records | Fix DNS using [not verifying](/docs/connect-domain/troubleshoot/not-verifying) |
| **suspended** | Domain disabled (policy, billing, or admin) | Contact support / fix account issues |

## Banner behavior

While status is `pending`, `verifying`, or `failed`, the domain page shows the DNS helper banner:

- **Auto populate** when Reloop discovers Domain Connect support for your host
- **Open DNS settings** / manual guidance otherwise

## Related

- [How verification works](/docs/connect-domain/verification/how-verification-works)
- [Propagation](/docs/connect-domain/verification/propagation)
