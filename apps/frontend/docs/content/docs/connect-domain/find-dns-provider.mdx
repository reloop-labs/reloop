...
- **Auto populate** on the domain page → use [Auto-populate](/docs/connect-domain/auto-populate)
- **Otherwise** → open that host’s guide under [Provider guides](/docs/connect-domain/providers) and add records manually
...
Then open the matching [provider guide](/docs/connect-domain/providers).
...
If the registrar is GoDaddy but NS are Cloudflare, **ignore GoDaddy’s DNS record editor** for Reloop. Edit Cloudflare. Full explanation: [Registrar vs DNS host](/docs/connect-domain/registrar-vs-dns-host).

## Still stuck?

Use the [Unknown provider](/docs/connect-domain/providers/unknown-provider) guide: open whatever panel controls the NS hosts you found, and paste the records from Reloop.
