...
- Access to the DNS panel for the **nameserver** host ([find your provider](/docs/connect-domain/find-dns-provider))
...
<Steps>
  ...
  <Step title="Open your DNS provider">
    Use **Open DNS settings** in Reloop if shown, or jump to the matching [provider guide](/docs/connect-domain/providers).
  </Step>
  ...
  <Step title="Merge SPF if needed">
    If an SPF TXT already exists, merge `include:reloop.sh` into it. Do not create a second SPF. See [SPF conflicts](/docs/connect-domain/troubleshoot/spf-conflicts).
  </Step>
  <Step title="Verify in Reloop">
    Return to Reloop → **Verify**. Allow time for [propagation](/docs/connect-domain/verification/propagation) if checks fail immediately.
  </Step>
</Steps>
...
- [ ] MX only changed if you intend to ([sending vs receiving](/docs/connect-domain/sending-vs-receiving))
- [ ] Tracking CNAME host is free or intentionally replaced

## Provider-specific guides

See the full list: [Provider guides](/docs/connect-domain/providers).
