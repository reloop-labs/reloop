...
Whoever runs those nameservers is your **DNS host** — that is where you add Reloop’s records. Often that is the same company as your registrar, but not always (see [Registrar vs DNS host](/docs/connect-domain/registrar-vs-dns-host)).
...
After you save a record, the internet does not update instantly everywhere. Caches honor **TTL**. Most providers update in minutes; worst case up to ~48–72 hours. Details: [DNS propagation](/docs/connect-domain/verification/propagation).
...
For a field-by-field map of Reloop’s records, see [DNS records explained](/docs/connect-domain/dns-records-explained).
