...
category: Tutorials
tags: [tutorial, disposable-email, email-validation, temp-email, api]
readTime: 5 min read
author:
  name: Farhan Shaikh
...
The [Reloop Temp Email Checker](https://reloop.sh/tools/temp-email-checker) is a thin wrapper over one public endpoint. `POST` an address to `https://reloop.sh/api/tools/v1/temp-email-checker` — no API key, no account.
...
Need more than disposable detection? Use Reloop's free [email validator](/tools/email-validator) to check syntax, role addresses, live MX records, and clean CSV lists (up to 1,000) in one pass.
...
For full list cleaning (syntax + disposable + MX), use the free **[Email Validator](/tools/email-validator)**.
