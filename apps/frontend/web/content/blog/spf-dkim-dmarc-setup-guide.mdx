...
**Email authentication is no longer optional—without SPF, DKIM, and DMARC properly configured, your transactional emails will consistently land in spam or be dropped by inbox providers.**
...
Reloop utilizes **dual selector-based CNAME records**. When you register a domain in the [Reloop dashboard](https://reloop.sh/dashboard/signup), Reloop automatically generates public/private key pairs and provides two CNAME DNS entries. 

**Because CNAME records point directly to Reloop-managed DNS infrastructure, Reloop handles key rotation and cryptographic hygiene automatically without requiring manual DNS updates.**

### What Reloop's DKIM records look like

You will receive two CNAME records structured as follows:
...
**Add both records exactly as displayed.** The selector prefixes (`reloop1`, `reloop2`) tell receiving mail servers precisely which public key record to retrieve from DNS.

### Verifying your DKIM records

To confirm that your DKIM CNAMEs resolve correctly to their underlying public keys:

```bash
dig CNAME reloop1._domainkey.yourdomain.com +short
dig TXT reloop1._domainkey.reloop.sh +short
```

You should see a TXT record returned containing `v=DKIM1; k=rsa; p=MIGf...`.

You can also send a test email to [mail-tester.com](https://mail-tester.com) or [check your raw message headers](/blog/why-emails-land-in-spam-fix) to inspect the evaluated `DKIM: PASS` status.

### Why DKIM sometimes fails after setup

- **DNS propagation delay**: DNS record changes can take anywhere from 15 minutes to 24 hours to propagate globally across resolver caches.
- **Accidental quote wrapping**: Some DNS dashboards automatically add quotes around record values. Ensure you are entering raw strings without extra surrounding quotes.
- **Hostname misconfiguration**: A frequent mistake is entering the full domain twice (e.g. `reloop1._domainkey.yourdomain.com.yourdomain.com`). If your DNS provider automatically appends your domain, enter only `reloop1._domainkey`.
...
- [ ] **Both DKIM CNAME records active**: Both `reloop1._domainkey` and `reloop2._domainkey` resolve correctly.
...
1. **Automated Key Generation**: Reloop automatically provisions RSA-2048 DKIM key pairs and publishes CNAME selectors.
