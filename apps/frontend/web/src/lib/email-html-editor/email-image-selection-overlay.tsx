...
import { setInlineCssDeclaration } from "./resolve-inspector-text-style";
...
export function EmailImageSelectionOverlay({
	...
	// Handle Image Replacement File Upload (offline / client-side FileReader)
	const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		reader.onload = (event) => {
			...
			if (dataUrl) {
				const resolvedUrl = ensureAbsoluteUrl(dataUrl);
				editor
					.chain()
					.setNodeSelection(targetPos)
					.updateAttributes("image", { src: resolvedUrl })
					.run();

				setSelectedImage((prev) =>
					prev ? { ...prev, src: resolvedUrl } : null,
				);
