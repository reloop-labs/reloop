...
const FRAMEWORKS_AI_PROMPT = `Integrate Reloop email into this project.
...
Do the following:
1. Detect this project's framework (Next.js, Express, NestJS, Fastify, Elysia, Django, FastAPI, Flask, Laravel, Rails, Spring Boot, ASP.NET Core, Phoenix, Gin).
2. Install the correct Reloop SDK if one exists (Node/Python: reloop-email; Go: github.com/reloop-labs/reloop-go/v2; PHP: reloop/reloop-email; Java: sh.reloop:reloop-email; .NET: Reloop.Email; Ruby: reloop-email; Elixir: reloop), or call the REST API.
...
export default function FrameworksIndexHero() {
	const [copied, setCopied] = useState(false);

	const handleCopyPrompt = async () => {
		try {
			await navigator.clipboard.writeText(FRAMEWORKS_AI_PROMPT);
			setCopied(true);
	...
	return (
		<section className="relative w-full max-w-full overflow-x-clip bg-bg-white-0 text-text-strong-950 dark:bg-black dark:text-white">
			<div className="mx-auto flex w-full max-w-5xl flex-col items-center border-stroke-soft-200 px-4 pt-28 pb-14 text-center sm:px-6 sm:pt-32 sm:pb-16 md:max-w-7xl md:items-start md:text-left lg:px-8 xl:border-x dark:border-white/10">
				...
				<p className="mt-3 max-w-2xl text-[14px] text-balance text-text-sub-600 leading-relaxed sm:mt-4 sm:text-[16px] dark:text-white/60">
