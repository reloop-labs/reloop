...
const analysis = await reloop.tools.checkBlocklist({
  target: 'reloop.sh',
});

console.log(\`Clean: \${analysis.isClean} (\${analysis.listedCount}/\${analysis.totalChecked} listed)\`);
...
analysis = client.tools.check_blocklist(
    target="reloop.sh"
)

print(f"Clean: {analysis.is_clean} ({analysis.listed_count}/{analysis.total_checked} listed)")
for entry in analysis.results:
    if entry.is_listed:
        print(f"LISTED on {entry.name}: {entry.delist_url}")`,
	},
	{
		slug: "go",
		name: "Go",
...
func main() {
	...
	fmt.Printf("Clean: %t (%d/%d listed)\\n", res.IsClean, res.ListedCount, res.TotalChecked)
}`,
	},
	{
		slug: "rust",
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let analysis = client.tools().check_blocklist(&BlocklistCheckRequest {
        target: "reloop.sh".into(),
    }).await?;

    println!("Clean: {} ({}/{} listed)", analysis.is_clean, analysis.listed_count, analysis.total_checked);
    Ok(())
}`,
	},
	{
...
$analysis = $reloop->tools->checkBlocklist([
    'target' => 'reloop.sh',
]);

echo "Clean: " . ($analysis->is_clean ? 'Yes' : 'No') . " ({$analysis->listed_count}/{$analysis->total_checked} listed)\\n";`,
	},
	{
		slug: "ruby",
		name: "Ruby",
...
puts "Clean: #{analysis.is_clean} (#{analysis.listed_count}/#{analysis.total_checked} listed)"`,
	},
	{
		slug: "elixir",
		name: "Elixir",
		...
		checkCode: `analysis = Reloop.Tools.check_blocklist(%{
  target: "reloop.sh"
})

IO.puts("Clean: #{analysis.is_clean} (#{analysis.listed_count}/#{analysis.total_checked} listed)")`,
	},
	{
		slug: "java",
		name: "Java",
...
public class App {
    public static void main(String[] args) {
        ...
        var analysis = reloop.tools().checkBlocklist(
            BlocklistCheckRequest.builder()
                .target("reloop.sh")
                .build()
        );

        System.out.println("Clean: " + analysis.isClean());
    }
}`,
	},
	{
...
var analysis = await client.Tools.CheckBlocklistAsync(new BlocklistCheckRequest {
    Target = "reloop.sh"
});

Console.WriteLine("Clean: " + analysis.IsClean + " (" + analysis.ListedCount + "/" + analysis.TotalChecked + " listed)");`,
	},
	{
		slug: "curl",
		name: "cURL",
		...
		checkCode: `curl -X POST https://api.reloop.sh/api/tools/v1/blocklist-check \\
		...
		<div className="w-full">
			...
			<div className="grid grid-cols-1 lg:grid-cols-12 border-b border-stroke-soft-200 dark:border-white/10">
				...
				<div className="px-6 py-6 sm:px-10 sm:py-7 lg:col-span-9 lg:px-12 lg:py-8">
					...
					<StepItem
						number={2}
						title={`Check DNSBL blocklists with ${active.name}`}
						isLast={false}
					>
						<SdkCodeBlock
							code={active.checkCode}
							slug={active.slug}
