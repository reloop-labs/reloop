...
const TOOL_LANGUAGES: ToolLanguage[] = [
	{
		...
		fileName: "check_senders.ts",
		checkCode: `import Reloop from 'reloop-email';

const reloop = new Reloop(process.env.RELOOP_API_KEY);

const report = await reloop.tools.whoSends({
  domain: 'stripe.com',
});

console.log(\`Inbox: \${report.inbox.provider}\`);
console.log(\`Verdict: \${report.verdict}\`);
console.log('Authorized Senders:', report.senders.map(s => s.vendor));`,
		frameworks: [],
	},
	{
		slug: "python",
		...
		fileName: "check_senders.py",
		checkCode: `import os
from reloop import Reloop

client = Reloop(api_key=os.environ["RELOOP_API_KEY"])

report = client.tools.who_sends(domain="stripe.com")

print(f"Inbox Provider: {report.inbox.provider}")
print(f"Headline: {report.headline}")
for sender in report.senders:
    print(f"- {sender.vendor} ({sender.role})")`,
		frameworks: [],
	},
	{
		slug: "go",
...
func main() {
	...
	report, err := client.Tools.WhoSends(context.Background(), &reloop.WhoSendsRequest{
		Domain: "stripe.com",
	})
	if err != nil {
		panic(err)
	...
	},
	{
		...
		fileName: "check_senders.rs",
		checkCode: `use reloop::{Client, WhoSendsRequest};

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let client = Client::from_env()?;

    let report = client.tools().who_sends(&WhoSendsRequest {
	...
	},
	{
		...
		fileName: "check_senders.php",
		checkCode: `<?php
use Reloop\\Reloop;

$reloop = new Reloop(getenv('RELOOP_API_KEY'));

$report = $reloop->tools->whoSends([
    'domain' => 'stripe.com',
]);
...
puts "Headline: #{report.headline}"`,
	...
	},
	{
		...
		fileName: "check_senders.exs",
		checkCode: `report = Reloop.Tools.who_sends(%{domain: "stripe.com"})

IO.puts("Headline: #{report.headline}")`,
		frameworks: [],
	},
	{
		slug: "java",
		...
		fileName: "CheckSenders.java",
		checkCode: `import sh.reloop.Reloop;
import sh.reloop.models.WhoSendsRequest;

public class App {
    public static void main(String[] args) {
        var reloop = new Reloop(System.getenv("RELOOP_API_KEY"));

        var report = reloop.tools().whoSends(
            WhoSendsRequest.builder().domain("stripe.com").build()
        );

        System.out.println("Headline: " + report.getHeadline());
    }
}`,
		frameworks: [],
	},
	{
		...
		fileName: "CheckSenders.cs",
		checkCode: `using Reloop;

var client = new ReloopClient(Environment.GetEnvironmentVariable("RELOOP_API_KEY")!);

var report = await client.Tools.WhoSendsAsync(new WhoSendsRequest {
    Domain = "stripe.com"
});

Console.WriteLine($"Headline: {report.Headline}");`,
		frameworks: [],
	},
	{
		slug: "curl",
		...
		fileName: "check_senders.sh",
		checkCode: `curl -X POST https://reloop.sh/api/tools/v1/who-sends \\
		...
		<div className="w-full">
			...
			<div className="grid grid-cols-1 border-stroke-soft-200 border-b lg:grid-cols-12 dark:border-white/10">
				...
				<div className="px-6 py-6 sm:px-10 sm:py-7 lg:col-span-9 lg:px-12 lg:py-8">
					...
					<StepItem
						number={2}
						title={`Query sender permissions with ${active.name}`}
						isLast={false}
					>
						<SdkCodeBlock
							code={sendCode}
					...
					<StepItem
						number={3}
						title="Inspect JSON sender fingerprint report (200 OK)"
						isLast={true}
					>
						<SdkCodeBlock
