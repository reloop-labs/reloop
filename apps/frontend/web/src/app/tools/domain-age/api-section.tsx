...
const TOOL_LANGUAGES: ToolLanguage[] = [
	{
		...
		fileName: "check_age.ts",
		checkCode: `import Reloop from 'reloop-email';

const reloop = new Reloop(process.env.RELOOP_API_KEY);

const report = await reloop.tools.domainAge({
  domain: 'stripe.com',
});

console.log(\`Age: \${report.age.ageDays} days\`);
...
report = client.tools.domain_age(domain="stripe.com")

print(f"Domain: {report.domain}")
print(f"Age: {report.age.age_days} days")
...
func main() {
	...
	report, err := client.Tools.DomainAge(context.Background(), &reloop.DomainAgeRequest{
		Domain: "stripe.com",
	})
	if err != nil {
		panic(err)
	...
	},
	{
		...
		fileName: "check_age.rs",
		checkCode: `use reloop::{Client, DomainAgeRequest};

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let client = Client::from_env()?;

    let report = client.tools().domain_age(&DomainAgeRequest {
        domain: "stripe.com".into(),
    }).await?;

    println!("Headline: {}", report.headline);
    println!("Age: {:?} days", report.age.age_days);
    Ok(())
}`,
		frameworks: [],
	},
	{
		...
		fileName: "check_age.php",
		checkCode: `<?php
use Reloop\\Reloop;

$reloop = new Reloop(getenv('RELOOP_API_KEY'));

$report = $reloop->tools->domainAge([
    'domain' => 'stripe.com',
]);

echo "Age: " . $report->age->ageDays . " days\\n";
...
puts "Age: #{report.age.age_days} days"
puts "Headline: #{report.headline}"`,
	...
	},
	{
		...
		fileName: "check_age.exs",
		checkCode: `report = Reloop.Tools.domain_age(%{domain: "stripe.com"})
...
IO.puts("Headline: #{report.headline}")`,
	...
	},
	{
		...
		fileName: "CheckAge.java",
		checkCode: `import sh.reloop.Reloop;
import sh.reloop.models.DomainAgeRequest;

public class App {
    public static void main(String[] args) {
        var reloop = new Reloop(System.getenv("RELOOP_API_KEY"));

        var report = reloop.tools().domainAge(
            DomainAgeRequest.builder().domain("stripe.com").build()
        );
...
Console.WriteLine($"Headline: {report.Headline}");`,
	...
	},
	{
		...
		fileName: "check_age.sh",
		checkCode: `curl -X POST https://reloop.sh/api/tools/v1/domain-age \\
		...
		<div className="w-full">
			...
			<div className="grid grid-cols-1 border-stroke-soft-200 border-b lg:grid-cols-12 dark:border-white/10">
				...
				<div className="px-6 py-6 sm:px-10 sm:py-7 lg:col-span-9 lg:px-12 lg:py-8">
					...
					<StepItem
						number={2}
						title={`Query domain age and warmup advice with ${active.name}`}
						isLast={false}
					>
						<SdkCodeBlock
							code={sendCode}
					...
					<StepItem
						number={3}
						title="Inspect JSON domain age report (200 OK)"
						isLast={true}
					>
						<SdkCodeBlock
