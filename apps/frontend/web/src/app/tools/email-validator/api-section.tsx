...
const result = await client.tools.email.verify("alex@reloop.sh");

console.log(result.health.state);   // "undeliverable"
console.log(result.health.score);   // 0
console.log(result.health.reason);  // "no_mx_records"
console.log(result.mailServer.implicitMxRecord); // true

// 2. Or submit an asynchronous batch CSV/list check
const batch = await client.tools.email.verifyBatch({
  emails: ["user1@gmail.com", "user2@mailinator.com"],
});
console.log(\`Batch token: \${batch.token}\`);`,
		frameworks: [],
	},
	{
		slug: "python",
...
# 1. Verify a single email address
result = client.tools.email.verify("alex@reloop.sh")

print(result.health.state)   # "undeliverable"
print(result.health.score)   # 0
print(result.health.reason)  # "no_mx_records"

# 2. Or submit an asynchronous batch check
batch = client.tools.email.verify_batch(emails=["user1@gmail.com", "user2@mailinator.com"])
...
func main() {
	...
	res, err := client.Tools.Email.Verify(context.Background(), "alex@reloop.sh")
	if err != nil {
		panic(err)
	}

	fmt.Printf("State: %s, Score: %d, Reason: %s\\n", res.Health.State, res.Health.Score, res.Health.Reason)
}`,
		frameworks: [],
	},
	{
...
$res = $client->tools->email->verify('alex@reloop.sh');
echo "State: " . $res->health->state . " (Score: " . $res->health->score . ")\\n";`,
		frameworks: [],
	},
	{
		slug: "ruby",
...
puts "State: #{res.health.state}, Score: #{res.health.score}"`,
	...
	},
	{
		...
		fileName: "VerifyEmail.java",
		checkCode: `import com.reloop.Reloop;
import com.reloop.models.EmailHealthResult;

public class VerifyEmail {
    public static void main(String[] args) {
        Reloop client = new Reloop(System.getenv("RELOOP_API_KEY"));
        EmailHealthResult result = client.tools().email().verify("alex@reloop.sh");
        System.out.println("State: " + result.getHealth().getState());
    }
}`,
		frameworks: [],
	},
...
var res = await client.Tools.Email.VerifyAsync("alex@reloop.sh");
Console.WriteLine($"State: {res.Health.State}, Score: {res.Health.Score}");`,
		frameworks: [],
	},
	{
		slug: "rust",
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let res = client.tools().email().verify("alex@reloop.sh").await?;
    println!("State: {}, Score: {}", res.health.state, res.health.score);
    Ok(())
}`,
		frameworks: [],
	},
	{
		...
		fileName: "verify_email.exs",
		checkCode: `client = Reloop.client(System.get_env("RELOOP_API_KEY"))
{:ok, res} = Reloop.Tools.Email.verify(client, "alex@reloop.sh")
IO.puts("State: #{res.health.state}, Score: #{res.health.score}")`,
		frameworks: [],
	},
	{
		slug: "curl",
		...
		fileName: "verify_email.sh",
		checkCode: `# 1. Single email address verification
curl -X POST https://reloop.sh/api/tools/v1/email-health-check \\
  -H "Content-Type: application/json" \\
  -d '{"email": "alex@reloop.sh"}'

# 2. Async batch list check (up to 1,000 emails)
curl -X POST https://reloop.sh/api/tools/v1/email-health-check/batch \\
  -H "Content-Type: application/json" \\
  -d '{"emails": ["contact@google.com", "test@mailinator.com"]}'`,
		frameworks: [],
	},
];
		...
		<div className="w-full">
			...
			<div className="border-stroke-soft-200 border-b dark:border-white/10">
				<div
					ref={containerRef}
					role="tablist"
					aria-label="Email health checker SDK languages"
			...
			<div className="grid grid-cols-1 border-stroke-soft-200 border-b lg:grid-cols-12 dark:border-white/10">
				...
				<div className="px-6 py-6 sm:px-10 sm:py-7 lg:col-span-9 lg:px-12 lg:py-8">
					...
					<StepItem
						number={2}
						title={`Verify email health with ${active.name}`}
						isLast={false}
					>
						<SdkCodeBlock
							code={sendCode}
					...
					<StepItem
						number={3}
						title="Inspect JSON email health report (200 OK)"
						isLast={true}
					>
						<SdkCodeBlock
