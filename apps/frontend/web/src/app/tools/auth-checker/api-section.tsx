...
const TOOL_LANGUAGES: ToolLanguage[] = [
	{
		...
		fileName: "check_auth.ts",
		checkCode: `import Reloop from 'reloop-email';

const reloop = new Reloop(process.env.RELOOP_API_KEY);

const report = await reloop.tools.authChecker({
  domain: 'stripe.com',
  selector: 's1', // Optional DKIM selector
});

console.log(\`Score: \${report.score}/100 (Grade: \${report.grade})\`);
console.log(\`DMARC Policy: p=\${report.dmarc.policy}\`);
console.log(\`SPF: \${report.spf.rawRecord}\`);
console.log(\`DKIM (\${report.dkim.selector}): \${report.dkim.keyLength}-bit RSA\`);`,
		frameworks: [
			{
				slug: "nodejs",
				name: "Node.js",
...
console.log(\`Grade: \${report.grade} (\${report.verdictLabel})\`);`,
			},
		],
	},
	{
		...
		fileName: "check_auth.py",
		checkCode: `import os
from reloop import Reloop

client = Reloop(api_key=os.environ["RELOOP_API_KEY"])

report = client.tools.auth_checker(
    domain="stripe.com",
    selector="s1"
)

print(f"Score: {report.score}/100 (Grade: {report.grade})")
print(f"DMARC Policy: {report.dmarc.policy}")
print(f"SPF Status: {report.spf.status}")`,
		frameworks: [
			{
				slug: "python",
				name: "Python",
...
func main() {
	...
	report, err := client.Tools.AuthChecker(context.Background(), &reloop.AuthCheckerRequest{
		Domain:   "stripe.com",
		Selector: "s1",
	})
	if err != nil {
	...
	fmt.Printf("DMARC: %s (RUA: %v)\\n", report.Dmarc.Policy, report.Dmarc.Rua)
}`,
		frameworks: [
			{
				slug: "go",
...
func main() {
	...
	report, _ := client.Tools.AuthChecker(context.Background(), &reloop.AuthCheckerRequest{
		Domain: "stripe.com",
	})
	...
	},
	{
		...
		fileName: "check_auth.rs",
		checkCode: `use reloop::{Client, AuthCheckerRequest};

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let client = Client::from_env()?;

    let report = client.tools().auth_checker(&AuthCheckerRequest {
        domain: "stripe.com".into(),
        selector: Some("s1".into()),
    }).await?;

    println!("Score: {}/100 (Grade: {})", report.score, report.grade);
    println!("DMARC Policy: {:?}", report.dmarc.policy);
    Ok(())
}`,
		frameworks: [],
	},
...
$report = $reloop->tools->authChecker([
    'domain' => 'stripe.com',
    'selector' => 's1',
]);

echo "Score: " . $report->score . " (Grade: " . $report->grade . ")\\n";
echo "DMARC: " . $report->dmarc->policy . "\\n";`,
		frameworks: [],
	},
	{
		slug: "ruby",
...
puts "Score: #{report.score} (Grade: #{report.grade})"
puts "DMARC Policy: #{report.dmarc.policy}"`,
		frameworks: [],
	},
	{
		slug: "elixir",
		...
		fileName: "check_auth.exs",
		checkCode: `report = Reloop.Tools.auth_checker(%{
  domain: "stripe.com",
  selector: "s1"
})

IO.puts("Score: #{report.score}/100 (Grade: #{report.grade})")
IO.puts("DMARC: #{report.dmarc.policy}")`,
		frameworks: [],
	},
	{
		slug: "java",
		...
		fileName: "CheckAuth.java",
		checkCode: `import sh.reloop.Reloop;
import sh.reloop.models.AuthCheckerRequest;

public class App {
    public static void main(String[] args) {
        var reloop = new Reloop(System.getenv("RELOOP_API_KEY"));

        var report = reloop.tools().authChecker(
            AuthCheckerRequest.builder()
                .domain("stripe.com")
                .selector("s1")
                .build()
        );

        System.out.println("Grade: " + report.getGrade() + " (Score: " + report.getScore() + ")");
    }
}`,
		frameworks: [],
	},
	{
		...
		fileName: "CheckAuth.cs",
		checkCode: `using Reloop;

var client = new ReloopClient(Environment.GetEnvironmentVariable("RELOOP_API_KEY")!);

var report = await client.Tools.AuthCheckerAsync(new AuthCheckerRequest {
    Domain = "stripe.com",
    Selector = "s1"
});

Console.WriteLine($"Score: {report.Score}/100 (Grade: {report.Grade})");
Console.WriteLine($"DMARC: {report.Dmarc.Policy}");`,
		frameworks: [],
	},
	{
		slug: "curl",
		...
		fileName: "check_auth.sh",
		checkCode: `curl -X POST https://reloop.sh/api/tools/v1/auth-checker \\
		...
		<div className="w-full">
			...
			<div className="grid grid-cols-1 border-stroke-soft-200 border-b lg:grid-cols-12 dark:border-white/10">
				...
				<div className="px-6 py-6 sm:px-10 sm:py-7 lg:col-span-9 lg:px-12 lg:py-8">
					...
					<StepItem
						number={2}
						title={`Audit email authentication records with ${active.name}`}
						isLast={false}
					>
						<SdkCodeBlock
							code={sendCode}
					...
					<StepItem
						number={3}
						title="Inspect JSON authentication report (200 OK)"
						isLast={true}
					>
						<SdkCodeBlock
