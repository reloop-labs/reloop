...
const TOOL_LANGUAGES: ToolLanguage[] = [
	{
		...
		fileName: "check_reputation.ts",
		checkCode: `import Reloop from 'reloop-email';

const reloop = new Reloop(process.env.RELOOP_API_KEY);

const report = await reloop.tools.domainReputation({
  domain: 'reloop.sh',
});

console.log(\`Reputation Score: \${report.score}/100 (\${report.grade})\`);
console.log(\`Verdict: \${report.verdict} - \${report.verdictLabel}\`);
console.log(\`Auth Score: \${report.breakdown.authentication.score}%\`);
console.log(\`Blocklists Clean: \${report.details.blocklist.cleanCount}/\${report.details.blocklist.totalChecked}\`);`,
		frameworks: [],
	},
	{
		slug: "python",
		...
		fileName: "check_reputation.py",
		checkCode: `import os
from reloop import Reloop

client = Reloop(api_key=os.environ["RELOOP_API_KEY"])

report = client.tools.domain_reputation(domain="reloop.sh")

print(f"Reputation Score: {report.score}/100 ({report.grade})")
print(f"Verdict: {report.verdict} - {report.verdict_label}")
print(f"Auth Score: {report.breakdown.authentication.score}%")
print(f"Blocklists Clean: {report.details.blocklist.clean_count}/{report.details.blocklist.total_checked}")`,
		frameworks: [],
	},
	{
		slug: "go",
...
func main() {
    ...
    report, err := client.Tools.DomainReputation(context.Background(), &reloop.DomainReputationParams{
        Domain: "reloop.sh",
    })
    if err != nil {
        panic(err)
	...
	},
	{
		...
		fileName: "CheckReputation.java",
		checkCode: `import sh.reloop.Reloop;
import sh.reloop.models.DomainReputationRequest;

public class App {
    public static void main(String[] args) {
        var reloop = new Reloop(System.getenv("RELOOP_API_KEY"));

        var report = reloop.tools().domainReputation(
            DomainReputationRequest.builder().domain("reloop.sh").build()
        );

        System.out.println("Score: " + report.getScore() + " (" + report.getGrade() + ")");
	...
	},
	{
		...
		fileName: "CheckReputation.cs",
		checkCode: `using Reloop;

var client = new ReloopClient(Environment.GetEnvironmentVariable("RELOOP_API_KEY")!);

var report = await client.Tools.DomainReputationAsync(new DomainReputationRequest {
    Domain = "reloop.sh"
});
...
Console.WriteLine($"Verdict: {report.Verdict}");`,
	...
	},
	{
		...
		fileName: "check_reputation.sh",
		checkCode: `curl -X POST https://reloop.sh/api/tools/v1/domain-reputation \\
		...
		<div className="w-full">
			...
			<div className="grid grid-cols-1 border-stroke-soft-200 border-b lg:grid-cols-12 dark:border-white/10">
				...
				<div className="px-6 py-6 sm:px-10 sm:py-7 lg:col-span-9 lg:px-12 lg:py-8">
					...
					<StepItem
						number={2}
						title={`Query domain reputation score with ${active.name}`}
						isLast={false}
					>
						<SdkCodeBlock
							code={sendCode}
					...
					<StepItem
						number={3}
						title="Inspect JSON reputation report (200 OK)"
						isLast={true}
					>
						<SdkCodeBlock
