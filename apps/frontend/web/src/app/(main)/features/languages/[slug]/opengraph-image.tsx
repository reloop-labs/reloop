...
export default async function Image({
	...
	const title = language
		? `Send Email with ${language.name} | Reloop`
		: "Send Email | Reloop";
	const description = language
		? language.shortDescription
		: "Open-source, self-hostable email infrastructure.";
	const primaryColor = "#d97757";

	const sdkInstall = language ? `SDK: ${language.name}` : "SDK Libraries";
	const codeExample = language
		? `import { Reloop } from "reloop-${slug}"`
			...
			>
				...
				>
					...
					>
						...
						<span
							style={{ color: "rgba(255, 255, 255, 0.5)", paddingLeft: "16px" }}
						>
							{sdkInstall}
						</span>

						<span
							style={{ color: "#d97757", fontWeight: 600, marginTop: "12px" }}
						>
							$ Code Example
						</span>
						<span style={{ color: "#a3e635", paddingLeft: "16px" }}>
							{codeExample}
						</span>
					</div>
				</div>
