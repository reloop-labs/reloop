...
export class AsciiWordmarkRenderer {
	...
	private initComposer(w: number, h: number, dpr: number) {
		...
		(this.pointsMat.uniforms["uResolution"]!.value as THREE.Vector2).set(
			w * dpr,
			h * dpr,
		);
	...
	private resize() {
		...
		const bw = Math.max(2, Math.round(w * RENDER_SCALE));
		const bh = Math.max(2, Math.round(h * RENDER_SCALE));
		(this.asciiPass.uniforms["uResolution"]!.value as THREE.Vector2).set(
			bw * dpr,
			bh * dpr,
		);
	...
	private loop = () => {
		...
		this.trailOn +=
			((this.onCard ? 1 : 0) - this.trailOn) * Math.min(1, dt * 6);
