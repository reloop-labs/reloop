...
const updateCards: UpdateCard[] = [
	...
	{
		title: "Blog",
		description: "Be the first to get new Reloop updates.",
		href: "/blog",
		external: false,
		iconName: "file-text",
	},
		...
		<section className="w-full border-stroke-soft-200 border-t bg-bg-white-0 text-text-strong-950 dark:border-white/10 dark:bg-black dark:text-white">
			<div className="mx-auto w-full max-w-5xl border-stroke-soft-200 border-x px-4 py-16 sm:px-6 md:max-w-7xl lg:px-8 lg:py-20 dark:border-white/10">
				<div className="text-left">
					<h2 className="font-semibold text-2xl text-text-strong-950 tracking-tight sm:text-3xl lg:text-[2rem] dark:text-white">
						Keep up to date.
					</h2>
					<p className="mt-2 text-[15px] text-text-sub-600 sm:text-base dark:text-white/60">
				...
				<div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 sm:gap-5">
					{updateCards.map((card) => {
						const cardClassName =
							"group flex flex-col justify-between rounded-2xl border border-stroke-soft-200 bg-white p-6 transition-all duration-200 hover:border-stroke-soft-300 hover:bg-bg-weak-50 dark:border-white/10 dark:bg-black dark:hover:border-white/20 dark:hover:bg-white/[0.04]";

						const content = (
							<>
								<div className="flex items-start justify-between">
									<div className="flex size-6 items-center justify-start text-text-strong-950 dark:text-white">
										<Icon
											name={card.iconName}
									...
									<Icon
										name="arrow-up-right"
										className="size-3.5 text-text-sub-600 opacity-0 transition-all duration-200 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:opacity-100 dark:text-white/60"
										aria-hidden="true"
									/>
								</div>
								<div className="mt-8">
									...
									<p className="mt-1.5 text-[13px] text-text-sub-600 leading-relaxed sm:text-[13.5px] dark:text-white/55">
										{card.description}
									</p>
								</div>
							</>
