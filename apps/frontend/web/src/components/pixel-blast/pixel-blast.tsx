...
// Integrated from React Bits (JavaScript + CSS variant), typed for this codebase.
...
interface TouchNormPoint {
	x: number;
	y: number;
}

interface TrailPoint extends TouchNormPoint {
	age: number;
	force: number;
	vx: number;
	vy: number;
...
interface TouchTexture {
	...
	addTouch: (norm: TouchNormPoint) => void;
	update: () => void;
	radiusScale: number;
	size: number;
}
...
const createTouchTexture = (): TouchTexture => {
	...
	let last: TouchNormPoint | null = null;
	const maxAge = 64;
	let radius = 0.1 * size;
	...
	const addTouch = (norm: TouchNormPoint) => {
		let force = 0;
		let vx = 0;
		let vy = 0;
		if (last) {
...
interface PixelBlastSession {
	...
	noiseEffect: Effect | undefined;
}

interface PixelBlastReinitConfig {
	antialias: boolean;
	liquid: boolean;
	noiseAmount: number;
}
...
export const PixelBlast = ({
	...
	const threeRef = useRef<PixelBlastSession | null>(null);
	const prevConfigRef = useRef<PixelBlastReinitConfig | null>(null);

	// Track on-screen visibility so the render loop can pause offscreen.
	useEffect(() => {
		...
		const observer = new IntersectionObserver(
			(entries) => {
				...
				visibilityRef.current.visible = entry ? entry.isIntersecting : true;
			},
			{ threshold: 0 },
		);
	...
	useEffect(() => {
		...
		const needsReinitKeys: (keyof PixelBlastReinitConfig)[] = [
			"antialias",
			"liquid",
			"noiseAmount",
		];
		const cfg: PixelBlastReinitConfig = { antialias, liquid, noiseAmount };
		...
		if (mustReinit) {
			...
			const setSize = () => {
				...
				const session = threeRef.current;
				if (session?.composer)
					session.composer.setSize(
						renderer.domElement.width,
						renderer.domElement.height,
					);
			...
			if (t) {
				...
				if (t.liquidEffect) {
					...
					const freqUniform = t.liquidEffect.uniforms.get("uFreq");
