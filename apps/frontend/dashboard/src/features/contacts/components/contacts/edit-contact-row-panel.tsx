	...
	return (
		...
		>
			<AnimatePresence mode="wait" initial={false}>
				{isPending ? (
					<div className="rounded-2xl border border-stroke-soft-100/80 bg-bg-white-0 p-8 shadow-regular-xs dark:border-stroke-soft-100/40">
						<motion.div
							key="loading"
							initial={{ opacity: 0 }}
							animate={{ opacity: 1 }}
							...
							className="flex items-center justify-center gap-2 text-paragraph-sm text-text-sub-600"
						>
							<Spinner size={20} />
					...
					<motion.div
						...
						exit={{ opacity: 0 }}
						transition={{ duration: 0.15 }}
						className="text-center text-error-base text-paragraph-sm"
					>
