...
export function ContactSelectionActionBar({
	table,
}: {
	table: Table<Contact>;
}) {
	...
	const runBulkStatus = async (status: AudienceStatus): Promise<void> => {
		...
		try {
			for (const contact of contacts) {
				try {
					...
					updateContactStatusInCache(contact.id, status);
					ok += 1;
				} catch {
					failed += 1;
				}
			...
			if (failed === 0) {
				toast.success(
					status === "subscribed"
						? `Subscribed ${ok} contact${ok === 1 ? "" : "s"}`
						: `Unsubscribed ${ok} contact${ok === 1 ? "" : "s"}`,
				);
			} else {
				toast.error(
					`${failed} of ${contacts.length} action${contacts.length === 1 ? "" : "s"} failed`,
				);
			}
		} finally {
			setBusy(false);
	...
	return (
		...
		>
			...
			<ActionBarGroup>
				<ActionBarItem
					disabled={busy}
					onClick={() => void runBulkStatus("subscribed")}
				>
				...
				<ActionBarItem
					disabled={busy}
					onClick={() => void runBulkStatus("unsubscribed")}
				>
					<Icon name="minus-circle" className="size-3.5" />
					Unsubscribe
				</ActionBarItem>
				<FancyButton.Root
					variant="destructive"
