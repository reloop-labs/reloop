...
export function DeleteGroupConfirmModal({
	...
	const currentGroup = groups.find((g) => g.id === deleteId);
	if (currentGroup) {
		targetGroupRef.current = currentGroup;
	}
	const groupToDelete = currentGroup || targetGroupRef.current;

	const targetBulkGroupsRef = useRef<Group[]>([]);
	if (isBulk && selectedGroups.length > 0) {
		targetBulkGroupsRef.current = selectedGroups;
	}
	const bulkGroupsToDelete = isBulk
		? selectedGroups.length > 0
			? selectedGroups
			: targetBulkGroupsRef.current
		: [];
	...
	const canDelete =
		isConfirmed &&
		deleteState === "idle" &&
		(isBulk ? bulkGroupsToDelete.length > 0 : !!groupToDelete);
	...
	const handleDelete = async () => {
		...
		try {
			...
			if (isBulk) {
				let ok = 0;
				let failed = 0;
				for (const group of bulkGroupsToDelete) {
					try {
						const response = await fetch(
							`/api/contacts/v1/groups/${group.id}`,
				...
				setTimeout(() => {
					...
					onClearSelection?.();
					onDeleteSuccess?.(`${ok} group${ok === 1 ? "" : "s"}`);
					if (failed > 0) {
						toast.error(
							`${failed} group${failed === 1 ? "" : "s"} failed to delete`,
						);
					}
					setTimeout(() => {
						setDeleteState("idle");
			...
			toast.error(
				error instanceof Error ? error.message : "Failed to delete group(s)",
			);
			setDeleteState("idle");
		}
	};
			...
			onOpenChange={(open) => {
				if (!open) {
					if (deleteStateRef.current === "success") {
						const name = isBulk
							? `${targetBulkGroupsRef.current.length || bulkGroupsToDelete.length} groups`
							: targetGroupRef.current?.name || "Unnamed group";
			...
			>
				<div>
					<Modal.Title className="font-semibold text-[26px] text-text-strong-950 tracking-tight">
						{isBulk
							? `Delete ${bulkGroupsToDelete.length || "selected"} groups`
							: "Delete group"}
					</Modal.Title>
					<Modal.Description className="text-sm text-text-sub-600 leading-relaxed">
						{isBulk
							? `Are you sure you want to delete ${bulkGroupsToDelete.length > 1 ? `these ${bulkGroupsToDelete.length} groups` : "this group"}? This action cannot be undone.`
				...
				{isBulk ? (
					<div className="mt-5 space-y-2 rounded-xl border border-stroke-soft-100 bg-bg-weak-50/50 p-4 dark:border-stroke-soft-100/40">
						<p className="font-normal text-text-sub-600 text-xs">
							Selected groups ({bulkGroupsToDelete.length})
						</p>
						<div className="flex flex-wrap gap-1.5 pt-1">
							{bulkGroupsToDelete.slice(0, 4).map((g) => (
								<span
									key={g.id}
							...
							{bulkGroupsToDelete.length > 4 ? (
								<span className="inline-flex items-center rounded-md bg-bg-weak-50 px-2 py-1 font-medium text-text-sub-600 text-xs dark:bg-bg-weak-50/30">
									+{bulkGroupsToDelete.length - 4} more
								</span>
							) : null}
						</div>
					</div>
