...
export const DeleteChannelModal = ({ channels }: DeleteChannelModalProps) => {
	...
	const {data: contactsData, isPending: isLoadingContacts} = useQuery({
		queryKey: ["contacts", "legacy", channelToDelete && isOpen
			? `/api/contacts/v1/channels/${channelToDelete.id}/contacts?limit=1`
			: null],
		queryFn: async () => {
			const url = channelToDelete && isOpen
			? `/api/contacts/v1/channels/${channelToDelete.id}/contacts?limit=1`
			: null;
	...
	// Reset confirmation when modal closes or channel changes
	useEffect(() => {
		if (!isOpen) {
			const timer = setTimeout(() => {
				setConfirmationText("");
	...
	useHotkeys(
		...
		(e) => {
			e.preventDefault();
			if (isOpen && confirmationText === channelToDelete?.name && !isDeleting) {
				handleDelete();
			}
		},
		{ enableOnFormTags: ["INPUT"], enabled: isOpen && !!channelToDelete },
		[isOpen, confirmationText, channelToDelete, isDeleting],
	);
	...
	const handleDelete = async () => {
		...
		if (confirmationText !== channelToDelete.name) {
			toast.error("Please enter the correct channel name to confirm deletion");
			return;
		}
				...
				<div className="rounded-20 border border-stroke-soft-100/50 bg-bg-white-0">
					{!channelToDelete && isOpen ? (
						...
						<form
							onSubmit={(e) => {
								e.preventDefault();
								if (confirmationText === channelToDelete.name && !isDeleting) {
									handleDelete();
								}
							}}
						>
