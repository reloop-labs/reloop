...
export function WorkflowSelectionActionBar({
	table,
	onBulkStatus,
	onDelete,
}: {
	table: Table<Workflow>;
	onBulkStatus?: (action: "active" | "paused") => Promise<void>;
	onDelete: () => void;
}) {
	...
	const handleStatusAction = async (status: "active" | "paused") => {
		if (selectedCount === 0 || busy || !onBulkStatus) return;
		setBusy(true);
		try {
			...
			<ActionBarGroup>
				{onBulkStatus ? (
					<>
						...
						<ActionBarItem
							disabled={busy}
							onClick={() => void handleStatusAction("paused")}
						>
				...
				>
					...
					Delete {selectedCount} automation{selectedCount === 1 ? "" : "s"}
