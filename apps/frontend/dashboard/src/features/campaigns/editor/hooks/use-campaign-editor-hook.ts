...
export const useCampaignEditorHook = (collab: CampaignCollabOptions) => {
	...
	const uploadImage = useCallback(async (file: File) => {
		...
		if (!response.ok) {
			...
			console.error("[useCampaignEditorHook] Upload error:", errText);
	...
	const editor = useEditor(
		{
			...
			onContentError(e) {
				console.error("[campaign-editor] content error", e);
			},
		},
		[collab.provider, collab.ydoc],
	);
