...
export function CampaignEmptyState() {
	...
	const isFiltered = statusFilters.length > 0 || searchQuery.trim() !== "";

	const handleClearFilters = () => {
		void setStatusFilters([]);
		void setSearchQuery("");
	...
	return (
		<div className="flex flex-col items-center px-6 py-12 text-center dark:bg-bg-weak-50/30">
			<div className="mb-4 flex items-center justify-center">
				<Icon
					name={isFiltered ? "search" : "mega-phone"}
					className="h-8 w-8 text-text-sub-600"
				/>
			</div>
			<h3 className="mb-2 font-semibold text-text-strong-950 text-xl">
				{isFiltered ? "No campaigns found" : "Create your first campaign"}
			</h3>
			<p className="mx-auto mb-6 max-w-75 text-balance font-medium text-[12px] text-text-sub-600">
				{isFiltered
					? "Try adjusting your search or filters."
					: "Broadcast newsletters and email updates to your contacts."}
			</p>
			{isFiltered ? (
				<Button.Root
					type="button"
