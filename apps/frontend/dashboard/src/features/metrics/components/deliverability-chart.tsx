...
export function DeliverabilityChart({
	...
	const { data, isPending: isLoading } = useEmailStatsQuery({
		startDate,
		endDate,
		domain,
	...
	const maxSentValue = useMemo(() => {
		...
		for (const key of keysToPlot) {
			const array = data[key as keyof typeof data] as number[] | undefined;
	...
	const avgRate = data
		? data.rate.reduce((a, b) => a + b, 0) / (data.rate.length || 1)
		: 0;
