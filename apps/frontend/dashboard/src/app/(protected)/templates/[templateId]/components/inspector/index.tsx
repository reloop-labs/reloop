...
function SectionHeader({
	...
	return (
		<button
			type="button"
			onClick={onToggle}
			className="flex w-full items-center justify-between py-2 font-semibold text-sm text-text-strong-950"
		>
			<span>{label}</span>
			{open ? (
				/* minus */
...
export const EmailInspector = () => {
	...
	const [openSpacing, setOpenSpacing] = useState(true);
								...
								<Inspector.Node>
									{({ nodeType, getAttr, setAttr }) =>
										nodeType === "button" ? (
											...
											<p className="py-2 text-text-soft-400 text-xs">
												Select a button to edit its link.
											</p>
										)
									}
								</Inspector.Node>
