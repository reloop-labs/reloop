...
interface SubscriberPreviewProps {
  topic: Topic;
  /** Other public topics in the same org to show realistic context */
  siblingTopics?: Topic[];
  orgName?: string;
  contactEmail?: string;
}
...
export function SubscriberPreview({
  ...
  // Build the list of topics to show in the preview: current topic + siblings (public only)
  const allPreviewTopics = [
    topic,
    ...siblingTopics.filter((t) => t.id !== topic.id && t.visibility === "public"),
  ].slice(0, 5); // cap at 5 for the preview

  const initialStates = Object.fromEntries(
    allPreviewTopics.map((t) => [t.id, t.defaultSubscription === "opt_in"]),
  );
  const [toggleStates, setToggleStates] = useState<Record<string, boolean>>(initialStates);
  const [theme, setTheme] = useState<"light" | "dark">("light");
  const [saving, setSaving] = useState<string | null>(null);

  const handleToggle = (topicId: string, val: boolean) => {
    setSaving(topicId);
    setTimeout(() => {
      setToggleStates((prev) => ({ ...prev, [topicId]: val }));
      setSaving(null);
    }, 400);
  };
                    ...
                    className={cn(
                      ...
                      isChecked
                        ? isDark
                          ...
                          : "bg-blue-50/40"
                        : "",
                      // Highlight the current topic being viewed
                      isCurrentTopic
                        ? isDark
          ...
          <p className="text-[10px] text-text-sub-600">
            {topic.visibility === "private" ? (
              <span className="flex items-center gap-1 text-warning-base">
                ...
                This topic is private — it won't appear on the preferences page
              </span>
            ) : (
              <span className="flex items-center gap-1 text-success-base">
                ...
                This topic is public — subscribers can see and manage it
              </span>
            )}
          </p>
          <span className="text-[10px] text-text-sub-600">
            {allPreviewTopics.filter((t) => toggleStates[t.id]).length} of{" "}
            {allPreviewTopics.length} subscribed
          </span>
        </div>
      </div>
    </div>
