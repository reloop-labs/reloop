...
export const ChannelBulkImportContent = () => {
	...
	} = useSWR<Channel>(`/api/contacts/v1/channels/${channelId}`, {
	...
	const handleImport = async () => {
		...
		try {
			...
			// Step 2: Get all contact IDs and bulk add to channel
	...
	if (channelError || !channelData) {
		return (
			<div className="flex flex-col items-center justify-center gap-4 p-8">
				...
				<h2 className="font-semibold text-xl">Failed to load channel</h2>
			...
			<div className="flex w-full items-center justify-between border-stroke-soft-200 border-b border-dashed pt-6 pb-6">
				<div>
					...
					<p className="text-paragraph-sm text-text-sub-600">
						Import up to 1000 contacts to "{channelData.name}" from a CSV file
					</p>
				</div>
			</div>
