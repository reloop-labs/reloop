...
export const AddContactToGroupModal = ({
	...
	const { data } = useSWR<{ contacts: Contact[] }>(
		open ? `/api/contacts/list?limit=100${debouncedSearch ? `&search=${encodeURIComponent(debouncedSearch)}` : ""}` : null,
	);
	...
	const { data: groupData } = useSWR<{ group: { contacts: Contact[] } }>(
		open && groupId ? `/api/contacts/v1/groups/${groupId}/contacts?limit=100` : null,
	);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			await mutate(
				(key: string) =>
					typeof key === "string" && key.includes(`/api/contacts/v1/groups/${groupId}/contacts`),
			);
		} catch (error) {
						...
						<Modal.Footer className="mt-4 flex items-center justify-end gap-3 border-stroke-soft-100/50">
							...
							>
								{isSubmitting ? (
									...
									<>
										Add {selectedContactIds.length > 0 ? selectedContactIds.length : ""} Contact
