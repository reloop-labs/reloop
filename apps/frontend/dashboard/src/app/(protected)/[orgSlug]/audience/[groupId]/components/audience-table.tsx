"use client";
import { useUserOrganization } from "@fe/dashboard/providers/org-provider";
import {
	getAnimationProps,
	getDisplayName,
	getStatusColorClass,
	getStatusIcon,
	getStatusLabel,
} from "@fe/dashboard/utils/audience";
import { formatRelativeTime } from "@fe/dashboard/utils/time";
import type { Audience } from "@reloop/api/types";
import * as Button from "@reloop/ui/button";
import { Icon } from "@reloop/ui/icon";
import {
	Content as PopoverContent,
	Root as PopoverRoot,
	Trigger as PopoverTrigger,
} from "@reloop/ui/popover";
import { Skeleton } from "@reloop/ui/skeleton";
import { AnimatePresence, motion } from "motion/react";
import Link from "next/link";
import { useQueryState } from "nuqs";

interface AudienceTableProps {
	audiences: Audience[];
	activeOrganizationSlug: string;
	currentAudienceId?: string;
	isLoading?: boolean;
	loadingRows?: number;
	onSubscribe?: (audienceId: string) => void;
	onUnsubscribe?: (audienceId: string) => void;
	onDelete?: (audienceId: string) => void;
}

export const AudienceTable = ({
	audiences,
	activeOrganizationSlug,
	currentAudienceId,
	isLoading,
	loadingRows = 3,
	onSubscribe,
	onUnsubscribe,
	onDelete,
}: AudienceTableProps) => {
	const { push } = useUserOrganization();
	const [, setDeleteId] = useQueryState("delete");

	const handleDeleteAudience = (audienceId: string) => {
		setDeleteId(audienceId);
		if (onDelete) {
			onDelete(audienceId);
		}
	};

	const handleViewDetails = (audienceId: string) => {
		push(`/audience/${audiences[0]?.audienceGroupId}/${audienceId}`);
	};

	const handleSubscribe = (audienceId: string) => {
		if (onSubscribe) {
			onSubscribe(audienceId);
		}
	};

	const handleUnsubscribe = (audienceId: string) => {
		if (onUnsubscribe) {
			onUnsubscribe(audienceId);
		}
	};

	return (
		<AnimatePresence mode="wait">
			<div className="w-full overflow-hidden rounded-xl border border-stroke-soft-200 text-paragraph-sm shadow-regular-md ring-stroke-soft-200 ring-inset">
				<div className="grid grid-cols-[1fr_minmax(200px,auto)_minmax(100px,auto)_minmax(40px,auto)]">
					<div className="bg-bg-weak-50 pl-5 font-medium text-text-sub-600">
						<div className="py-2.5">Audience</div>
					</div>
					<div className="bg-bg-weak-50 font-medium text-text-sub-600">
						<div className="py-2.5">Status</div>
					</div>
					<div className="bg-bg-weak-50 font-medium text-text-sub-600">
						<div className="py-2.5">Added</div>
					</div>
					<div className="bg-bg-weak-50 font-medium text-text-sub-600">
						<div className="py-2.5" />
					</div>
					{isLoading
						? // Skeleton loading state
							Array.from({ length: loadingRows }).map((_, index) => (
								<div key={`skeleton-${index}`} className="group/row contents">
									<div className="flex items-center border-stroke-soft-200 border-t py-2.5">
										<div className="my-1 pl-5">
											<Skeleton className="h-4 w-32" />
										</div>
									</div>
									<div className="flex items-center border-stroke-soft-200 border-t py-2.5">
										<div className="flex items-center gap-2">
											<Skeleton className="h-2 w-2 rounded-full" />
											<Skeleton className="h-4 w-16" />
										</div>
									</div>
									<div className="flex items-center border-stroke-soft-200 border-t py-2.5">
										<Skeleton className="h-4 w-20" />
									</div>
									<div className="flex items-center border-stroke-soft-200 border-t py-2.5">
										<Skeleton className="h-4 w-4" />
									</div>
								</div>
							))
						: audiences.map((audience, index) => (
								<div key={`audience-${index}`} className="group/row contents">
									<div className="flex items-center border-stroke-soft-200 border-t py-2.5 group-hover/row:bg-bg-weak-50">
										<motion.div
											{...getAnimationProps(index + 1, 0)}
											className="flex items-center gap-2 pl-5"
										>
											<Link
												href={`/${activeOrganizationSlug}/audience/${audience.audienceGroupId}/${audience.id}`}
												className={`flex items-center gap-2 transition-colors hover:text-blue-600 ${
													currentAudienceId === audience.id
														? "text-blue-600"
														: ""
												}`}
											>
												<Icon
													name="user"
													className="h-4 w-4 text-text-sub-600"
												/>
												<div className="flex flex-col">
													<span className="font-medium text-label-sm text-text-strong-950">
														{getDisplayName(
															audience.firstName,
															audience.lastName,
															audience.email,
														)}
													</span>
													<span className="text-label-xs text-text-sub-600">
														{audience.email}
													</span>
												</div>
											</Link>
										</motion.div>
									</div>
									<div className="flex items-center border-stroke-soft-200 border-t py-2.5 group-hover/row:bg-bg-weak-50">
										<motion.div
											{...getAnimationProps(index + 1, 1)}
											className="flex items-center gap-2"
										>
											<div
												className={`flex items-center gap-2.5 rounded-lg py-0.5 pl-3 font-medium text-label-xs capitalize ${getStatusColorClass(audience.status)}`}
											>
												<Icon
													name={getStatusIcon(audience.status)}
													className="h-3.5 w-3.5"
												/>
												{getStatusLabel(audience.status)}
											</div>
										</motion.div>
									</div>
									<div className="flex items-center border-stroke-soft-200 border-t py-2.5 group-hover/row:bg-bg-weak-50">
										<motion.span
											{...getAnimationProps(index + 1, 2)}
											className="text-label-sm text-text-strong-950"
										>
											{formatRelativeTime(audience.addedAt)}
										</motion.span>
									</div>
									<div className="flex items-center border-stroke-soft-200 border-t py-2.5 group-hover/row:bg-bg-weak-50">
										<motion.div
											{...getAnimationProps(index + 1, 3)}
											className="flex items-center justify-center"
										>
											<PopoverRoot>
												<PopoverTrigger asChild>
													<Button.Root
														variant="neutral"
														mode="ghost"
														size="xxsmall"
														className="rounded p-1"
													>
														<Icon
															name="more-vertical"
															className="h-4 w-4 text-text-sub-600 hover:text-text-strong-950"
														/>
													</Button.Root>
												</PopoverTrigger>
												<PopoverContent align="end" className="w-48 p-2">
													<div className="flex flex-col gap-1">
														<Button.Root
															variant="neutral"
															mode="ghost"
															size="small"
															onClick={() => handleViewDetails(audience.id)}
															className="w-full justify-start"
														>
															<Icon name="eye-outline" className="h-4 w-4" />
															View Details
														</Button.Root>
														{audience.status === "subscribed" ? (
															<Button.Root
																variant="neutral"
																mode="ghost"
																size="small"
																onClick={() => handleUnsubscribe(audience.id)}
																className="w-full justify-start"
															>
																<Icon name="minus-circle" className="h-4 w-4" />
																Unsubscribe
															</Button.Root>
														) : (
															<Button.Root
																variant="neutral"
																mode="ghost"
																size="small"
																onClick={() => handleSubscribe(audience.id)}
																className="w-full justify-start"
															>
																<Icon name="check-circle" className="h-4 w-4" />
																Subscribe
															</Button.Root>
														)}
														<Button.Root
															variant="error"
															mode="ghost"
															size="small"
															onClick={() => handleDeleteAudience(audience.id)}
															className="w-full justify-start text-red-600 hover:bg-red-50"
														>
															<Icon name="trash" className="h-4 w-4" />
															Delete
														</Button.Root>
													</div>
												</PopoverContent>
											</PopoverRoot>
										</motion.div>
									</div>
								</div>
							))}
				</div>
			</div>
		</AnimatePresence>
	);
};
