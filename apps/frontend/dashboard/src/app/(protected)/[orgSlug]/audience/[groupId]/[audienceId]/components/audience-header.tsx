"use client";
import { getStatusColorClass, getStatusIcon, getStatusLabel } from "@fe/dashboard/utils/audience";
...
export const AudienceHeader = ({ audience, isLoading }: AudienceHeaderProps) => {
	if (isLoading) {
		return (
			<div className="mb-8">
	...
	if (!audience) {
		return (
			<div className="mb-8">
				...
				<p className="text-text-sub-600">The requested audience could not be found.</p>
			</div>
		);
	}
			...
			<div className="flex gap-6">
				...
				{audience.unsubscribedAt && (
					<div className="flex items-center gap-2">
						...
						<span className="text-sm text-text-sub-600">
							Unsubscribed {new Date(audience.unsubscribedAt).toLocaleDateString()}
						</span>
					</div>
				)}
			</div>
