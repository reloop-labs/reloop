  ...
  if (isLoading) {
    return (
      <div className="pt-10 pb-6">
        <Button.Root
          onClick={() => router.push(`/${activeOrganization.slug}/topics`)}
          variant="neutral"
          mode="stroke"
          size="xxsmall"
        >
      ...
      <div className="mt-6 flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            <Icon name="notification-indicator" className="h-5 w-5 text-text-sub-600" />
            <h1 className="font-medium text-2xl">{topic.name}</h1>
          </div>
          {topic.description && (
            <p className="mt-1 text-sm text-text-sub-600">{topic.description}</p>
          )}
        </div>
        <div className="flex items-center gap-2">
          <PopoverRoot>
