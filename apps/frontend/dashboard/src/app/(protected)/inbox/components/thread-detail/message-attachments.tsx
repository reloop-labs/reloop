...
export const MessageAttachments = ({
	...
	return (
		<div className="border-stroke-soft-100 border-t px-5 py-4 dark:border-stroke-soft-100/10">
			<h3 className="mb-3 font-medium text-xs text-text-sub-600">
				Attachments
			</h3>
			<ul className="flex flex-col gap-2">
				{attachments.map((file: any) => (
					<li key={file.name}>
						...
						>
							<Icon name="file" className="h-4 w-4 shrink-0 text-text-sub-600" />
							<div className="min-w-0 flex-1">
								<p className="truncate text-xs font-medium text-text-strong-950 dark:text-neutral-300">
									{file.name}
								</p>
								<p className="text-[11px] text-text-soft-400">{file.size}</p>
							</div>
							<Icon name="file-download" className="h-4 w-4 text-text-soft-400" />
						</button>
					</li>
				))}
			</ul>
