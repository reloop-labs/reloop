...
export function PageTOCPopoverTrigger(props: ComponentProps<"button">) {
	...
	return (
		<CollapsibleTrigger
			{...props}
			className={cn(
				"flex h-(--fd-tocnav-height) w-full items-center gap-2.5 px-4 py-2.5 text-start text-fd-muted-foreground text-sm focus-visible:outline-none md:px-6 [&_svg]:size-4 [&_svg]:shrink-0",
				props.className,
			)}
		>
			<ProgressCircle
			...
			{/* <ChevronDown
        className={cn("mx-0.5 transition-transform", open && "rotate-180")}
      /> */}
		</CollapsibleTrigger>
	);
}
