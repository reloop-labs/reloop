...
export function Sidebar({
				...
				<Presence present={open}>
					{({ present }) => (
						<aside
							...
							data-state={state}
							className={cn(
								"fixed inset-y-2 start-2 z-40 flex w-[85%] max-w-[380px] flex-col rounded-2xl border bg-fd-background text-[15px] shadow-lg data-[state=closed]:animate-fd-sidebar-out data-[state=open]:animate-fd-sidebar-in",
								!present && "invisible",
								props.className,
							)}
						>
	...
	return (
		<aside
			...
			className={cn(
				"fixed start-0 top-(--fd-sidebar-top) bottom-(--fd-sidebar-margin) z-20 flex flex-col items-end border-e bg-fd-card text-sm *:w-(--fd-sidebar-width) max-md:hidden",
				collapsed && [
					"rounded-xl border",
					hover
...
export function SidebarFolderContent(props: CollapsibleContentProps) {
	...
	return (
		<CollapsibleContent {...props} className={cn("relative", props.className)}>
			...
			>
				{ctx.level === 1 && (
					<div className="absolute inset-y-1 start-2.5 w-px bg-fd-border" />
				)}
				{props.children}
			</Context.Provider>
		</CollapsibleContent>
