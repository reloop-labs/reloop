...
export async function sendCampaignMail(params: {
	...
	const url = `${campaignsConfig.BASE_URL.replace(/\/$/, "")}/api/mail/v1/send`;
	const res = await fetch(url, {
		method: "POST",
		headers: {
			"content-type": "application/json",
			"user-agent": "reloop-campaigns/1.0",
			[INTERNAL_SECRET_HEADER]: campaignsConfig.RELOOP_INTERNAL_SECRET,
