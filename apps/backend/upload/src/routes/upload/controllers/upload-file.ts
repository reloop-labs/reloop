...
export async function uploadFile(params: {
    ...
    try {
        ...
        // Create directory structure: uploads/{organizationId}/{year}/{month}/
        const now = new Date();
        const year = now.getFullYear();
        const month = String(now.getMonth() + 1).padStart(2, "0");
        const uploadDir = `uploads/${organizationId}/${year}/${month}`;
        ...
        return {
            ...
            url: fileUrl,
            organizationId: newUpload[0].organizationId,
        ...
        if (error instanceof Error && error.message.includes("Invalid file type")) {
            throw status(400, { message: "Invalid file type. Only images are allowed" });
        }
