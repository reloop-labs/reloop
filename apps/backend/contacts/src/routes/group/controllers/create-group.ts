...
import { createGroupId } from "@reloop/db/id";
...
export const createGroup = async({
  ...
  GroupTypes.GroupResponse | GroupModel.Unauthorized
  const { name } = body;
logger.info(
  { organizationId: activeOrganizationId, name },
  "Creating group",
);
try {
...
export async function createGroupHandler(
  ...
  return await createGroup(params, logger);
}
