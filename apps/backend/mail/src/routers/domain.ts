...
import { DomainService, type AddDomainRequest } from "../lib/domain-service";
...
export const domainRouter = new Elysia()
      ...
      tags: [
        {
          ...
          description: "Operations for managing mail domains, DKIM keys, and DNS records"
        }
      ]
    },
    path: "/swagger"
  ...
  .post(
    ...
    {
      body: t.Object({
        domain: t.String({ minLength: 1, description: "Domain name (e.g., example.com)" }),
        serverIP: t.String({ 
          minLength: 7, 
        ...
        mailboxes: t.Optional(t.Number({ 
          minimum: 1, 
          maximum: 1000, 
          description: "Maximum number of mailboxes (default: 50)" 
        })),
        mailboxQuota: t.Optional(t.Number({ 
          minimum: 1073741824, 
      ...
      detail: {
        ...
        description: "Add a new domain with DKIM keys, DNS records, and admin account. If domain already exists, returns existing configuration.",
