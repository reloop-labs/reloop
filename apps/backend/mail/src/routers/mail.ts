...
export const mailRouter = new Elysia().post(
	"/send",
	async ({ body }) => {
		try {
	...
	},
	{
		body: t.Object({
			...
			to: t.Union(
				[t.String({ format: "email" }), t.Array(t.String({ format: "email" }))],
				{
		...
		detail: {
			...
			description:
				"Send an email through the mail server with built-in validation and authentication. Supports plain text, HTML, and template-based emails with DKIM signing.",
			tags: ["Mail Service"],
			examples: [
				{
