...
export class MailService {
	...
	private static getTransporter(): Transporter {
		if (!MailService.transporter) {
			MailService.transporter = createTransport({
	...
	static async sendMail(request: SendMailRequest): Promise<SendMailResponse> {
		try {
			...
			const domainRecord = await db
				.select()
				.from(domains)
				.where(and(eq(domains.domain, fromDomain), eq(domains.active, true)))
				.limit(1);
			...
			if (password) {
				...
				const mailboxRecord = await db
					...
					.where(
						and(
							...
							eq(mailboxes.domain, fromDomain),
				...
				const isValidPassword = MailService.verifyPassword(
					password,
					mailboxRecord[0].password,
				);
				if (!isValidPassword) {
					return {
						success: false,
