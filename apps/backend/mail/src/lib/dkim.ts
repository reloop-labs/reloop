...
export class DKIMGenerator {
	...
	static generateAllDNSRecords(
		...
		return [
			DKIMGenerator.generateARecord(domain, serverIP),
			DKIMGenerator.generateMXRecord(domain),
			DKIMGenerator.generateSPFRecord(domain, serverIP),
			DKIMGenerator.generateDMARCRecord(domain),
		];
	}
