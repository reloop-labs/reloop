...
export async function sendEmailController({
	...
	try {
		...
		if (!dnsHealthCheck.isHealthy) {
			throw MailErrors.dnsHealthError(
				domainName,
				dnsHealthCheck.missingRecords,
			);
		}
		...
		log.info(
			"server",
			`Email sending failed, refunding ${recipientCount} credits to organization ${organizationId}`,
		);
		try {
			await db
				.update(organization)
			...
			log.error(
				"server",
				`Failed to refund credits: ${refundError instanceof Error ? refundError.message : String(refundError)}`,
			);
		}
		throw error;
	}
