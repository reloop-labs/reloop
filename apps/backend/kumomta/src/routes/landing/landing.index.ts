...
`;
  ...
  .get("/agent-card.json", () => ({
    ...
    version: "1.0.0",
    description: "High-performance Mail Transfer Agent (MTA) interface for sending and tracking emails.",
    url: "https://reloop.sh",
    ...
    skills: [
      {
        id: "health_check",
        name: "Health Check",
        description: "Verify the connection status of KumoMTA and its database.",
        method: "GET",
      ...
      },
      {
        ...
        description: "Validate an API key and domain for internal routing purposes.",
        method: "POST",
        path: "/api/kumomta/v1/verify",
        tags: ["internal"],
        inputSchema: {
          key: { type: "string", required: true, description: "API Key to verify" },
          domain: { type: "string", required: true, description: "Domain to check" }
        },
        outputSchema: {
          userId: { type: "string" },
