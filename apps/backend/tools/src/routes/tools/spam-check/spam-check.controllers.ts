...
const CATEGORY_META: Record<TriggerCategory, { label: string }> = {
	urgency: { label: "Urgency" },
	shady: { label: "Shady & Deceptive" },
	overpromise: { label: "Overpromise & Hype" },
	money: { label: "Financial & Money" },
	outreach: { label: "Cold Outreach" },
};
...
export function checkSpamController(
	subjectInput: string,
	bodyInput: string,
): SpamCheckResult {
	...
	if (!subject) {
		...
		if (/^(?:re:|fwd:|fw:)/i.test(subject)) {
			...
			issues.push({
				...
				detail:
					"False reply prefixes violate CAN-SPAM regulations and trigger spam filters.",
		...
		for (const shortener of SUSPICIOUS_SHORTENERS) {
			if (lower.includes(shortener)) {
				...
				issues.push({
					...
					detail:
						"Spam filters heavily penalize generic link shorteners like bit.ly.",
...
export async function rewriteSpamCopyController(
	subjectInput: string,
	bodyInput: string,
): Promise<{ subject: string; body: string; provider: string }> {
	const geminiApiKey =
		toolsConfig.TOOLS_GEMINI_API_KEY || process.env.TOOLS_GEMINI_API_KEY || "";

	if (!geminiApiKey) {
		throw createError({
			status: 503,
			message: "AI rewrite service is currently unavailable",
			why: "No TOOLS_GEMINI_API_KEY environment variable is configured.",
			fix: "Please configure TOOLS_GEMINI_API_KEY in apps/backend/tools/.env.",
		});
	}

	try {
		const prompt = `You are an expert email deliverability engineer. Rewrite the following email subject and body to eliminate spam triggers, maximize inbox placement on Gmail and Outlook, and preserve the sender's original message.

Subject: ${subjectInput || "No subject provided"}
Body:
${bodyInput || "No body provided"}

Respond ONLY with valid JSON with the exact keys "subject" and "body". Do not include markdown formatting or extra text.
{"subject": "...", "body": "..."}`;
		...
		if (!response.ok) {
			let errMsg = `AI service returned status ${response.status}`;
			try {
		...
		if (!text) {
			throw new Error("No response generated by AI service.");
		}
		...
		if (!parsed.subject && !parsed.body) {
			throw new Error("Invalid output received from AI service.");
		}

		return {
			subject: parsed.subject || subjectInput,
			body: parsed.body || bodyInput,
			provider: "gemini-1.5-flash",
		};
	} catch (err: unknown) {
		throw createError({
			status: 502,
			message: "AI rewrite request failed",
			why:
				err instanceof Error
					? err.message
					: "Failed to connect to AI rewrite service.",
			fix: "Please check your network connection and try again.",
		});
	}
}
