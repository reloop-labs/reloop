...
export async function initBillingSubscribers() {
	// Payment Failed
	await bus.subscribe(BusEvent.PAYMENT_FAILED, async (payload) => {
		try {
			const html = await render(
				React.createElement(PaymentFailedEmail, {
					...
					nextRetryDate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toLocaleDateString(),
			...
			logger.error({ error, payload }, "Failed to send payment failed email");
		}
	});

	// Quota Warning
	await bus.subscribe(BusEvent.QUOTA_WARNING, async (payload) => {
		try {
			const html = await render(
			...
			logger.error({ error, payload }, "Failed to send quota warning email");
		}
	});

	// Trial Ending
	await bus.subscribe(BusEvent.TRIAL_ENDING, async (payload) => {
		try {
			const html = await render(
				React.createElement(TrialEndingEmail, {
					fullName: "User",
					daysLeft: payload.daysLeft,
					trialEndDate: new Date(Date.now() + payload.daysLeft * 24 * 60 * 60 * 1000).toLocaleDateString(),
