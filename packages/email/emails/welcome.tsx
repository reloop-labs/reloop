import * as React from "react";
import {
	Body,
	Button,
	Head,
	Heading,
	Hr,
	Html,
	Img,
	Preview,
	Section,
	Tailwind,
	Text,
} from "react-email";
import { Footer } from "../components/footer";

interface WelcomeEmailProps {
	fullName: string;
	baseUrl?: string;
}

const defaultBaseUrl = process.env.NEXT_PUBLIC_BASE_URL
	? `https://${process.env.NEXT_PUBLIC_BASE_URL}`
	: "https://reloop.sh";

export const WelcomeEmail = ({
	fullName = "User",
	baseUrl = defaultBaseUrl,
}: WelcomeEmailProps) => {
	const firstName = fullName ? fullName.split(" ").at(0) : "";
	const previewText = `Welcome to Reloop, ${firstName}! Building software is easy. Sending emails shouldn't be hard.`;

	return (
		<Html>
			<Head />
			<Preview>{previewText}</Preview>
			<Tailwind>
				<Body className="m-0 bg-[#0e0e0e] p-0 font-sans text-white">
					<Section className="mx-auto mt-[40px] mb-[40px] max-w-[560px] px-6">
						{/* Logo */}
						<Section className="mb-8">
							<Img
								src={`${baseUrl}/web-app-manifest-192x192.png`}
								width="32"
								height="32"
								alt="Reloop Logo"
								style={{ filter: "invert(1)" }}
							/>
						</Section>

						{/* Small Label */}
						<Text className="m-0 font-mono text-[#707070] text-[10px] uppercase tracking-[0.2em]">
							Welcome to Reloop
						</Text>

						{/* Main Headline */}
						<Heading
							className="mt-6 mb-8 p-0 font-normal font-serif text-[32px] text-white leading-[1.2]"
							style={{ fontFamily: "Georgia, serif" }}
						>
							Building software is easy.
							<br />
							<span className="text-[#707070]">
								Sending emails shouldn't be hard.
							</span>
						</Heading>

						<Hr className="my-8 border-[#222222]" />

						{/* Salutation & Intro */}
						<Text className="text-[15px] text-white leading-[1.6]">
							Hi {firstName},
						</Text>
						<Text className="mt-4 text-[#b0b0b0] text-[15px] leading-[1.6]">
							Welcome aboard. We're genuinely glad you're here.
						</Text>
						<Text className="mt-4 text-[#b0b0b0] text-[15px] leading-[1.6]">
							A new era of software is being built — AI agents that run
							autonomously, indie developers shipping products overnight, and
							marketing teams that move at startup speed. The builders are
							getting faster. The tools around them haven't kept up.
						</Text>
						<Text className="mt-4 text-[#b0b0b0] text-[15px] leading-[1.6]">
							That's what Reloop is here to fix.
						</Text>

						{/* Mission Section */}
						<Section className="mt-10 mb-10 border-[#222222] border-l border-solid pl-6">
							<Text className="m-0 font-mono text-[#707070] text-[10px] uppercase tracking-[0.2em]">
								Our Mission
							</Text>
							<Text className="mt-4 font-serif text-[18px] text-white italic leading-[1.6]">
								"Open-source email infrastructure built for AI agents, marketing
								teams, and developers — so you can focus on what you're
								building, not on how to reach the people who need it."
							</Text>
						</Section>

						<Text className="mt-8 text-[#b0b0b0] text-[15px] leading-[1.6]">
							We've seen the next generation of companies — smaller teams,
							bigger ambitions. They'll be powered by AI, built in the open, and
							run by founders who care more about their product than their
							billing stack.
						</Text>
						<Text className="mt-4 text-[#b0b0b0] text-[15px] leading-[1.6]">
							Reloop gives them the email layer they deserve: reliable,
							composable, and transparent — from transactional sends to
							AI-triggered sequences to developer APIs.
						</Text>

						{/* Capabilities Box */}
						<Section className="mt-10 rounded-lg border border-[#222222] border-solid p-8">
							<Text className="m-0 font-mono text-[#707070] text-[10px] uppercase tracking-[0.2em]">
								What you can do with Reloop
							</Text>

							<table
								width="100%"
								cellPadding="0"
								cellSpacing="0"
								style={{ marginTop: "24px" }}
							>
								<tr>
									<td style={{ verticalAlign: "top", width: "32px" }}>
										<Text className="m-0 font-mono text-[#404040] text-[10px]">
											01
										</Text>
									</td>
									<td
										style={{
											borderBottom: "1px solid #222222",
											paddingBottom: "24px",
										}}
									>
										<Text className="m-0 font-semibold text-white">
											AI Agents
										</Text>
										<Text className="mt-1 text-[#707070] text-[13px]">
											Trigger, compose, and send emails programmatically — your
											agents communicate like humans.
										</Text>
									</td>
								</tr>
								<tr>
									<td
										style={{
											verticalAlign: "top",
											width: "32px",
											paddingTop: "24px",
										}}
									>
										<Text className="m-0 font-mono text-[#404040] text-[10px]">
											02
										</Text>
									</td>
									<td
										style={{
											borderBottom: "1px solid #222222",
											paddingTop: "24px",
											paddingBottom: "24px",
										}}
									>
										<Text className="m-0 font-semibold text-white">
											Marketing Teams
										</Text>
										<Text className="mt-1 text-[#707070] text-[13px]">
											Campaign infrastructure with no vendor lock-in. Open,
											auditable, and fast to ship.
										</Text>
									</td>
								</tr>
								<tr>
									<td
										style={{
											verticalAlign: "top",
											width: "32px",
											paddingTop: "24px",
										}}
									>
										<Text className="m-0 font-mono text-[#404040] text-[10px]">
											03
										</Text>
									</td>
									<td style={{ paddingTop: "24px" }}>
										<Text className="m-0 font-semibold text-white">
											Developers
										</Text>
										<Text className="mt-1 text-[#707070] text-[13px]">
											Open-source APIs and SDKs. Self-host or use our cloud. You
											own your stack.
										</Text>
									</td>
								</tr>
							</table>
						</Section>

						{/* CTA Button */}
						<Section className="mt-10">
							<Button
								className="rounded bg-[#edece1] px-6 py-3 text-center font-bold font-mono text-[12px] text-black uppercase tracking-wider no-underline"
								href={`${baseUrl}/dashboard`}
							>
								Get Started &rarr;
							</Button>
						</Section>

						<Text className="mt-10 text-[#b0b0b0] text-[15px] leading-[1.6]">
							We're building Reloop in the open. That means you get a voice in
							what gets built next. If you have questions, feedback, or just
							want to say hi — reply to this email. We read everything.
						</Text>

						<Hr className="mt-10 mb-8 border-[#222222]" />

						<Text className="text-[#b0b0b0] text-[15px] leading-[1.6]">
							With love,
							<br />
							The Reloop Team
						</Text>

						<Footer baseUrl={baseUrl} />
					</Section>
				</Body>
			</Tailwind>
		</Html>
	);
};

export default WelcomeEmail;
