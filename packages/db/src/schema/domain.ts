...
export const domainTypeEnum = pgEnum("domain_type", ["custom", "subdomain", "system"]);
export const domainStatusEnum = pgEnum("domain_status", ["start-verify", "verifying", "active", "suspended", "failed"]);
export const dnsRecordTypeEnum = pgEnum("dns_record_type", ["A", "AAAA", "CNAME", "MX", "TXT", "NS", "SRV", "CAA", "SPF", "DKIM", "DMARC"]);
...
export const domain = pgTable(
    "domain",
    {
        ...
        dkimSelector: varchar("dkim_selector", { length: 255 }).notNull().default("reloop"),
        dmarcRecord: text("dmarc_record"),
        dmarcPolicy: varchar("dmarc_policy", { length: 50 }).notNull().default("none"),
...
export const domainDnsRecord = pgTable(
    ...
    (table) => [
        ...
        index("domain_dns_record_idx_domain_type").on(table.domainId, table.recordType),
        index("domain_dns_record_idx_domain_verified").on(table.domainId, table.isVerified),
        index("domain_dns_record_idx_domain_active").on(table.domainId, table.isActive),
        unique("domain_dns_record_unique_record").on(table.domainId, table.recordType, table.name, table.value),
    ],
);
...
export const domainDnsRecordRelations = relations(domainDnsRecord, ({ one }) => ({
    domain: one(domain, {
