...
export const webhook = pgTable(
    "webhook",
    {
        ...
        filteringOptions: jsonb("filtering_options").$type<Record<string, unknown>>(),
...
export const webhookDelivery = pgTable(
    ...
    (table) => [
        ...
        index("webhook_delivery_idx_webhook_status").on(table.webhookId, table.status),
        index("webhook_delivery_idx_webhook_created").on(table.webhookId, table.createdAt),
        index("webhook_delivery_idx_retry_pending").on(table.status, table.nextRetryAt),
    ],
);
